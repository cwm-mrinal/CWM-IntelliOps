...
def send_email_reply(ticket_id, from_emails, to_emails, cc_emails, reply_text):
    ...
    # Input validation - Updated to support multiple recipients in from_emails
    if not from_emails or not isinstance(from_emails, list):
        return {
            "statusCode": 400,
            "body": json.dumps({"error": "from_emails must be a non-empty list."})
        }
    ...
    logger.info("Sending email to recipients: %s", to_recipients)
